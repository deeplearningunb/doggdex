...
def load_dataset(path, shuffle=True):
    ...
    labels = np_utils.to_categorical(data['target'], dog_breeds)
    return files, labels

# load train and test datasets
train_files, train_labels = load_dataset('dataset/training_dataset/train')
test_files, _ = load_dataset('dataset/test_dataset/test', shuffle=False)

# save the labels of the train set
np.save('utils/train_labels.npy', train_labels)
...
np.save(open('utils/bottleneck_features_train.npy', 'wb'), bottleneck_features_train)
bottleneck_features_test = model.predict(preprocess_input(test_tensors))
np.save(open('utils/bottleneck_features_test.npy', 'wb'), bottleneck_features_test)
